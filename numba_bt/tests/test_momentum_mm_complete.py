...
def test_momentum_mm_complete():
    ...
    # 2. 读取aggtrade数据（复用test_timed_hedge_with_funding.py的逻辑）
    print(f"\n步骤2: 读取aggtrade数据")
    import glob
    import zipfile
    import duckdb
    ...
    if len(accounts) > 0:
        final_pos = accounts[-1, 2]
        final_cash = accounts[-1, 1]
        final_price = accounts[-1, 4]
        final_equity = final_cash + final_pos * final_price
        
        print(f"\n  仓位统计:")
        print(f"    最终仓位: {final_pos:.4f}")
        print(f"    最终现金: {final_cash:.2f}")
        print(f"    最终权益: {final_equity:.2f}")
        
        # 统计订单类型
        order_types = accounts[:, 9]
        taker_trades = np.sum(order_types == 0)
        taker_orders = np.sum(order_types == 1)
        maker_trades = np.sum(order_types == 2)
        funding_payments = np.sum(order_types == 6)
        
        print(f"\n  订单统计:")
        print(f"    Taker成交: {taker_trades} 次")
        print(f"    Taker下单: {taker_orders} 次")
        print(f"    Maker成交: {maker_trades} 次")
        print(f"    资金费支付: {funding_payments} 次")
        
        # 仓位变化范围
        positions = accounts[:, 2]
        max_pos = np.max(positions)
        min_pos = np.min(positions)
        print(f"    仓位范围: [{min_pos:.4f}, {max_pos:.4f}]")
        
        # 检查仓位是否在预期范围内
        pos_values = positions * accounts[:, 4]  # 仓位价值
        max_pos_value = np.max(np.abs(pos_values))
        print(f"    最大仓位价值: {max_pos_value:.2f}")
        print(f"    是否超过exposure (10000): {'是' if max_pos_value > 10000 else '否'}")
