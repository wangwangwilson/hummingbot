...
class BacktestingDataProvider(MarketDataProvider):
    ...
    def get_candles_df(self, connector_name: str, trading_pair: str, interval: str, max_records: int = 500):
        ...
        if candles_df is None or candles_df.empty:
            # 如果数据不存在，返回空DataFrame
            return pd.DataFrame(columns=["timestamp", "open", "high", "low", "close", "volume"])
        # 过滤时间范围
        filtered_df = candles_df[(candles_df["timestamp"] >= self.start_time) & (candles_df["timestamp"] <= self.end_time)]
        return filtered_df
    ...
    def ensure_epoch_index(df: pd.DataFrame, timestamp_column: str = "timestamp",
        ...
        if df.index.name == index_name or df.empty:
            # 如果已经是正确的索引名称，但类型是float，转换为int64以兼容切片操作
            if df.index.name == index_name and pd.api.types.is_numeric_dtype(df.index) and not pd.api.types.is_integer_dtype(df.index):
                df.index = df.index.astype('int64')
            return df

        # DatetimeIndex → convert to seconds (保持原有逻辑，但确保结果为int64)
        if isinstance(df.index, pd.DatetimeIndex):
            # pd.Timestamp.timestamp返回float，需要转换为int64以兼容切片操作
            df.index = df.index.map(pd.Timestamp.timestamp).astype('int64')
        # Has timestamp column → use as index (保持原有逻辑)
        elif timestamp_column in df.columns:
            df = df.set_index(timestamp_column, drop=not keep_original)
            # Convert non-numeric indices to seconds (保持原有逻辑，但确保结果为int64)
            if not pd.api.types.is_numeric_dtype(df.index):
                # pd.Timestamp.timestamp返回float，需要转换为int64以兼容切片操作
                df.index = pd.to_datetime(df.index).map(pd.Timestamp.timestamp).astype('int64')
        else:
            raise ValueError(f"Cannot create timestamp index: no '{timestamp_column}' column found and index isn't convertible")
        df.sort_index(inplace=True)
        df.index.name = index_name
        # 确保索引是int64类型（兼容性修复：pandas整数索引切片需要int64，不能是float）
        # pd.Timestamp.timestamp返回float，所以需要强制转换为int64
        if pd.api.types.is_numeric_dtype(df.index) and not pd.api.types.is_integer_dtype(df.index):
            df.index = df.index.astype('int64')
        elif pd.api.types.is_integer_dtype(df.index) and df.index.dtype != 'int64':
            # 如果是其他整数类型（如int32），也转换为int64
            df.index = df.index.astype('int64')
        return df
