...
class BacktestingEngineBase:
    ...
    async def run_backtesting(self,
        ...
        executors_info = await self.simulate_execution(trade_cost=trade_cost, show_progress=show_progress)
    ...
    async def simulate_execution(self, trade_cost: float, show_progress: bool = False) -> list:
        ...
        if show_progress and use_tqdm:
            iterator = tqdm(processed_features.iterrows(), total=total_rows, 
                          desc="回测进度", unit="行", 
                          ncols=100, mininterval=1.0, maxinterval=5.0,
                          bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]')
        elif show_progress:
            # 简单的进度显示
            iterator = self._simple_progress(processed_features.iterrows(), total_rows)
        else:
        ...
        for pos_idx, (i, row) in enumerate(iterator):
            ...
            if "features" in self.controller.processed_data and not self.controller.processed_data["features"].empty:
                ...
                if "timestamp" in features_df.columns or features_df.index.name == "timestamp":
                    ...
                    # 使用merge_asof找到最接近的features行
                    if len(features_df) > 0:
                        # 找到最接近当前时间戳的features行
                        features_df_sorted = features_df.sort_values("timestamp")
                        idx = features_df_sorted["timestamp"].searchsorted(current_ts, side='right') - 1
                        if idx >= 0 and idx < len(features_df_sorted):
                            feature_row = features_df_sorted.iloc[idx]
                            # 更新processed_features中当前行的reference_price和spread_multiplier
                            # 使用位置索引i（时间戳）来访问，因为processed_features的索引是时间戳
    ...
    def _simple_progress(self, iterator, total: int):
        """简单的进度显示（不使用tqdm）"""
        import sys
        import time
        ...
        for idx, item in enumerate(iterator):
            ...
            # 每处理1%或每5秒更新一次进度
            if idx == 0 or idx == total - 1 or (current_time - last_update) >= 5 or (idx % (total // 100 + 1) == 0):
                ...
                sys.stderr.write(f'\r回测进度: [{bar}] {progress:.1f}% | {idx+1}/{total} | '
                               f'速度: {rate:.0f} 行/秒 | 已用: {elapsed:.0f}秒 | 剩余: {eta:.0f}秒')
                sys.stderr.flush()
    ...
    async def update_state(self, row):
        ...
        # 关键修复：先调用controller的update_processed_data来更新特征数据
        # 然后再更新processed_data，确保reference_price和spread_multiplier是最新的
        await self.controller.update_processed_data()
        # 更新processed_data，但保留controller更新后的reference_price和spread_multiplier
        row_dict = row.to_dict()
        # 如果controller已经更新了reference_price和spread_multiplier，保留它们
    ...
    def summarize_results(executors_info: List, total_amount_quote: float = 1000):
        if len(executors_info) > 0:
            ...
            # 修复：使用filled_amount_quote > 0判断是否有持仓，而不是net_pnl_quote != 0
            ...
            total_volume = executors_with_position["filled_amount_quote"].sum() if len(executors_with_position) > 0 else 0.0
            ...
            # 修复：使用filled_amount_quote > 0判断是否有持仓
            executors_with_position = executors_df[executors_df["filled_amount_quote"] > 0].copy()
            # Additional metrics
            ...
            max_draw_down = np.min(drawdown) if len(drawdown) > 0 else 0.0
            # 修复边界检查：确保inventory不为空
            if len(executors_with_position) > 0 and len(executors_with_position["inventory"]) > 0:
                initial_inventory = executors_with_position["inventory"].iloc[0]
                max_drawdown_pct = max_draw_down / initial_inventory if initial_inventory != 0 else 0.0
            else:
                max_drawdown_pct = 0.0
            
            # 修复边界检查：确保cumulative_volume不为0
            if len(executors_with_position) > 0:
                cumulative_volume = executors_with_position["cumulative_volume"]
                cumulative_returns_col = executors_with_position["cumulative_returns"]
                returns = pd.to_numeric(
                    cumulative_returns_col / cumulative_volume.replace(0, np.nan)
                ).fillna(0)
